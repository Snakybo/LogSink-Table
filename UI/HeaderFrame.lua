...
StaticPopupDialogs["LOGSINK_ADD_COLUMN"] = {
    ...
    OnAccept = function(self)
		...
		if #key > 0 then
            Addon.Window:AddColumn(key, self.column)
			return false
        end
...
--- @return HeaderFrame
function HeaderFrame.Create(parent, columns)
	local base = CreateFrame("Frame", nil, parent)

	local result = Mixin(base, HeaderFrame)
...
function HeaderFrame:UpdateColumns()
...
--- @private
--- @param button HeaderButtonFrame
function HeaderFrame:CreateResizer(button)
	local EPSILON = 0.001
...
--- @private
--- @param frame HeaderButtonFrame
function HeaderFrame:HeaderButton_OnEnter(frame)
	frame.highlight:Show()
end

--- @private
--- @param frame HeaderButtonFrame
function HeaderFrame:HeaderButton_OnLeave(frame)
...
--- @param frame HeaderButtonFrame
--- @param button string
function HeaderFrame:HeaderButton_OnMouseUp(frame, button)
